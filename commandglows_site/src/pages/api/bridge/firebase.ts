...
function logTrialRequest(
  ...
  status: number,
  riskSignal: 'shared_network_velocity' | null = null
) {
  console.info(
    JSON.stringify({
      requestId,
  ...
  return value
    ...
    .map((entry) => {
      ...
      return {
        provider,
        providerAccountId:
          parseNullableString(raw.providerAccountId) ?? undefined,
...
function parseBridgeEntitlements(value: unknown): {
  ...
  return value
    ...
    .map((entry) => {
      ...
      const trialStartedAt = parseEpochMs(raw.trialStartedAt)
      const trialExpiresAt = parseEpochMs(raw.trialExpiresAt)
...
function parseBridgeSnapshot(value: unknown): BridgeSnapshot {
  ...
  return {
    ...
    replayGlowsProductUserId: parseNullableString(raw.replayGlowsProductUserId),
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const requestId =
    validRequestId && incomingRequestId ? incomingRequestId : randomUUID()
  let trialOutcome = 'unknown'
  let trialReasonCode: string | null = null
  let trialRiskSignal: 'shared_network_velocity' | null = null
  ...
  if (!bearerToken) {
    return respond(
      { status: 'unauthorized', error: 'missing_bearer_token' },
      401
    )
  }
  ...
  try {
    ...
    if (isTrialStart) {
      console.warn(
        JSON.stringify({
          requestId,
          action: 'start',
          diagnostic: 'firebase_token_verification_failed',
          errorCode: code,
        })
      )
    } else {
    ...
    return respond(
      { status: 'unauthorized', error: 'invalid_firebase_token' },
      401
    )
  }
  ...
  if (bridgeRequest.trialAction) {
    let emailVerified: boolean
    try {
      // Read current Firebase state instead of trusting a potentially stale
      // email_verified claim cached in an otherwise valid ID token.
      emailVerified = (await firebaseAdmin.auth.getUser(decodedToken.uid))
        .emailVerified
    } catch {
      return respond(
        {
          status: 'unavailable',
          error: 'firebase_email_verification_check_unavailable',
        },
        503
      )
    }
    if (!emailVerified) {
      trialOutcome = 'denied'
      trialReasonCode = 'email_not_verified'
      return respond({ status: 'denied', error: 'email_not_verified' }, 403)
    }
  }
  ...
  try {
    ...
    const rawTrialRequest =
      isTrialStart && 'trialRequest' in rawSnapshot
        ? (rawSnapshot as { trialRequest?: Record<string, unknown> })
            .trialRequest
        : undefined
    ...
    if (
      ...
      return respond(
        { status: 'error', error: 'invalid_trial_request_outcome' },
        502
      )
    }
    trialRiskSignal =
      rawTrialRequest?.riskSignal === 'shared_network_velocity'
        ? 'shared_network_velocity'
        : null
    ...
    if (isTrialStart) {
      ...
      const errorCode =
        ...
        /^(firestore_rest_write_failed:\d{3}|firebase_wif_[a-z0-9_]+|bridge_secret_(?:not_configured|mismatch)|unsupported_firestore_value|invalid_firestore_[a-z_]+)$/i.test(
          error.message
        )
          ? error.message
          : 'unknown'
      console.warn(
        JSON.stringify({
          requestId,
          action: 'start',
          diagnostic: 'bridge_write_failed',
          errorCode,
        })
      )
