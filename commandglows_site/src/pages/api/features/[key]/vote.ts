...
export const POST: APIRoute = async ({ request, locals, params }) => {
  ...
  if (!key) {
    return jsonResponse(
      { status: 'invalid', error: 'missing_feature_key' },
      400
    )
  }
  ...
  if (
    ...
    return jsonResponse(
      { status: 'unavailable', error: 'roadmap_unavailable' },
      503
    )
  }
