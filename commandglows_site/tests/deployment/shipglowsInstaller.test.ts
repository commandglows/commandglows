...
describe('ShipGlows public installer', () => {
  test('serves the generated shell artifact instead of a duplicated template', () => {
    ...
    expect(windowsInstaller).not.toContain("[ValidateSet('local','full')]")
    expect(windowsInstaller).toContain("$InstallMode -notin @('local', 'full')")
    expect(windowsInstaller).toContain("Read-Host 'Choose 1 or 2'")
    expect(windowsInstaller).toContain("'' { Write-Warn 'A choice is required. Enter 1, 2, or 0.' }")
    expect(windowsInstaller).not.toContain("Read-Host 'Choose 1 or 2 [2]'")
    expect(windowsInstaller).not.toContain("'' { return 'full' }")
