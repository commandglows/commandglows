...
function context(incomingOrigin: string | null = origin) {
  return { request: new Request(`${origin}/api/features/suggest`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...(incomingOrigin ? { Origin: incomingOrigin } : {}) }, body: JSON.stringify(payload) }), locals: { auth: () => ({ userId: 'clerk-user' }), siteAuth: () => ({ userId: 'global-verified' }) }, params: { key: 'feature' } }
}
...
test.each([null, 'https://evil.example'])('cookie-authorized admin mutation rejects untrusted origin %s', async (incomingOrigin) => {
  expect((await licenses(context(incomingOrigin) as never)).status).toBe(403)
...
test.each([suggest, vote])('feature mutations use server canonical identity and authority', async (route) => {
  ...
  expect(mutation).toHaveBeenCalledWith(expect.stringMatching(/^features:/), expect.objectContaining({ clerkId: 'clerk-user' }))
})
