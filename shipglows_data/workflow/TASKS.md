...
AppSumo progress (2026-09-10, same unified-commerce chantier): signed protocol parser and server-only OAuth exchange client implemented; 9 synthetic tests pass. Not yet connected to HTTP endpoints or Convex grants. Remaining AppSumo work: authenticated single-use OAuth state, durable provider license lineage, configurable tier mapping, webhook persistence/ACK, API reconciliation and integration tests. This is not a sellable AppSumo activation flow yet. PR #4 remains draft.

🔄 [Suite] task: Harmoniser le commerce, l'identité et les entitlements autour du ledger Convex canonique, avec CommunityGlows Windows comme premier pilote complet | status: in progress | area: unified-commerce-entitlements | id: unified-suite-commerce-entitlements | impact: critical | effort: large | authority: `C:\Users\Diane\ShipGlows\shipglows\shipglows_data\workflow\specs\unified-suite-commercial-entitlement-and-stripe.md` v1.3.1 + plan opérateur validé le 2026-09-06 | current_slice: socle commerce central vérifié localement — processeur unique, handoff/PaymentIntent, isolation, reçus immuables et reprise interne auditée | local_evidence: baseline 9016d47 revalidée (20 tests), tranche 151 tests/18 suites, typage Convex et Astro check | remaining: identité/recovery transverse, AppSumo, CommunityGlows Windows, miroirs et adaptateurs portfolio; aucune preuve hosted/provider/native revendiquée; aucun déploiement ni donnée fournisseur réelle
...
| ✅ | Rework core documentation path governance to remove compatibility doc files at repo root and use canonical `shipglows_data` paths | ✅ done |
...
<!-- central-shipglows-data-retirement: imported projects/winglowz/TASKS.md -->
