...
updated: "2026-09-24"
updated_at: "2026-09-24 11:45:57 UTC"
status: ready
...
- `reasonCode` on denial: `installation_not_eligible`,
  ...
  `active_paid_access`.
- `temporary_rate_limit` is scoped to the product, bridge environment, and a
  server-HMAC of the client network address. It allows up to three trial grants
  per network in each fixed 24-hour window across identities, so first-time
  email addresses can receive this denial because of grants to other accounts.
  Customer copy must identify the shared-network scope without suggesting that
  the email itself already used an essay.
