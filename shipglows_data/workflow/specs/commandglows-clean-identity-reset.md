...
updated: "2026-08-03"
updated_at: "2026-08-04 23:45:20 UTC"
status: ready
...
- Before → after: WinGlowz/WinGlows and legacy legacy identity domains → CommandGlows and `www.commandglows.com`; `diane-defores/winglowz` → `diane-defores/commandglows`; `winglowz_site/` and `winglowz_app/` → `commandglows_site/` and `commandglows_app/`; `winglowz-monorepo` → `commandglows-monorepo`; old package/bundle identities → `commandglows_app` and `com.commandglows.app`.
...
- Case variants and near-spellings: `WinGlowz`, `WinGlows`, `Winglowz`, `winglowz`, `winglows`, `legacy`, uppercase env names, hyphenated/underscored slugs, filenames, paths, URLs, emails, package names, and binary/channel identifiers.
...
- [ ] AC 1: Active source and configuration contain no unintended WinGlowz, WinGlows, winglowz, winglows, WINGLOWZ, WINGLOWS, the former public domain, or winglows.com references outside the allowlist.
...
| 2026-08-04 22:27:00 UTC | 006-sg-design | GPT-5.6 Codex | Diagnosed why the operator still saw no motion on the home navbar: the wordmark had been mistakenly included in the broad reduced-motion selector added during the prior change. Removed only that selector inclusion so the explicitly requested historical gradient animation is always visible on the home logo; typography, dimensions, white core, palette, and glow remain untouched. | complete | Verify the visible movement on the published home navbar after cache refresh. |
| 2026-08-04 22:35:00 UTC | 006-sg-design | GPT-5.6 Codex | Root-caused the persistent oversized dark contour in the CSS actually served by production: a later responsive `clamp(-5px, -1vw - 3px, …)` overrode the shared tight glow inset and expanded it to `-5px` on normal screens. Replaced the override with the canonical shared inset token, preserving the same compact glow in every responsive state and theme. | complete | Publish this source correction, then inspect the home navbar at desktop and mobile widths. |
| 2026-08-04 22:44:59 UTC | 108-sg-browser | GPT-5.6 Codex | Browser proof preflight correctly blocked: the active Playwright MCP config points to a missing Chromium 1228 executable while only Chromium 1217 is installed. Production source verification nevertheless confirmed that the new Vercel deployment loads `global.CLGyptIa.css`, which contains the canonical compact `inset: var(--text-logo-glow-inset-sm)` and no legacy responsive `-5px` clamp. | partial | Reload or repair the Playwright MCP runtime, then collect visual desktop/mobile proof in an actual browser. |
| 2026-08-04 23:45:20 UTC | 006-sg-design | GPT-5.6 Codex | Reverted the remaining full-word blurred gradient backdrop after operator review: it was the dark-mode “plaque” rather than a glyph glow. The backdrop element remains only for the existing pointer interaction, but is not rendered; the animated gradient, white face edge, and glow that follows individual glyphs remain unchanged and shared across light and dark. | partial | Publish and verify the home navbar; collect browser proof after the Playwright MCP runtime is repaired. |
| 2026-08-06 15:00:00 UTC | 010-sg-technical | GPT-5 Codex | Migrated the non-empty production Polar product configuration from the legacy name to `POLAR_COMMANDGLOWS_PRODUCT_ID`, then removed all six `*_LEGACY_*` commerce variables. The five Lemon Squeezy legacy variables and the Lemon Squeezy API key were empty, so no product/variant identifier could be migrated or inspected. Temporary local secret material was deleted after the operation. | partial | Configure the Lemon Squeezy API key and five CommandGlows product/variant IDs, then authorize a production deployment and hosted checkout/webhook proof. |
| 2026-08-06 15:05:00 UTC | 010-sg-technical | GPT-5 Codex | Recreated the five empty Lemon Squeezy product/variant configuration keys under their `LEMONSQUEEZY_COMMANDGLOWS_*` names after operator confirmation. No legacy commerce key was restored. | partial | Fill the API key and new product/variant values, then authorize a production deployment and hosted checkout/webhook proof. |
| 2026-08-06 15:30:00 UTC | 010-sg-technical | GPT-5 Codex | Updated user-facing Vercel, Clerk, Firebase, GitHub and global pnpm CLIs. The current Vercel CLI can now read the non-empty Lemon Squeezy key, but the provider rejects it with HTTP 401; format-only verification confirms that it lacks the expected `lsv2_` API-key prefix. No secret value was logged. | partial | Replace `LEMONSQUEEZY_API_KEY` with a current Lemon Squeezy API key, then enumerate products/variants and configure the five offer variables. |
...
| 006-sg-design | partial | `BRAND-CMD-001` is implemented across tokenized site wordmarks and web/native icon assets. The operator’s reported dark “plaque” is structurally removed: the full-word blurred backdrop is not rendered in either theme, leaving the animated gradient, white face edge, and per-glyph glow intact. Visual desktop/mobile proof is blocked until the stale Playwright MCP executable path is repaired. | Publish the correction, then collect visual proof in an actual browser. |
