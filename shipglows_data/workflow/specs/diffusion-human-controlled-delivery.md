...
Lots 0–3 et API/Flutter locale raccordés pour approbation humaine, arrêt/reprise, réduction contrôlée et lecture honnête des incidents → helper de politique analytics fermé par défaut testé mais non raccordé; agrégation persistée toujours bloquée par l'attribution à une tentative, le checkpoint et le choix de collecte/rétention → commit `f30420eb` poussé, Convex Dev et Prod déployés, Vercel Preview et Prod Ready, garde HTTP sans secret refusée → Convex Dev sans variables email et Convex Prod avec profil fermé sans client ni entreprise; aucun envoi réel → mapping Vercel Preview vers Convex Dev et synchronisation des variables non prouvés → garder l'envoi de production fermé jusqu'au profil approuvé, à la complétude des preuves analytics et à la clôture de l'inventaire d'exclusion/rollback historique.
