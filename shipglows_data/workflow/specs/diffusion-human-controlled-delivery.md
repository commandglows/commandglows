...
La synchronisation Doppler/Convex/Vercel des variables Dev/Prod a été refusée par le contrôle d'approbation de l'outil; aucun secret n'a été modifié pendant cette reprise. Le déploiement Vercel Preview manuel a échoué sur la racine de projet puis l'essai depuis le dépôt a été arrêté pendant l'empaquetage. La cible web Dev et son mapping Convex ne sont donc pas validés. L'activation reste bloquée par le profil métier, l'attribution/couverture analytique, la rétention et l'exclusion opérationnelle des anciens workers décrits plus haut.
