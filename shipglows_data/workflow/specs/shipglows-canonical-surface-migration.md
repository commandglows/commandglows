...
| 001-sg-build | implemented | Delegated regression-first repair passed 8 focused tests, shell syntax across sh/dash/busybox, Astro check, and a dynamic no-TTY safety scenario; public page wording still needs deployment proof. | Ship the bounded copy/test delta and verify the matching hosted pages. |
...
| 103-sg-verify | partial | The autossh repair passes local correctness and safety proof, and the public script hash matches source; the new EN/FR prerequisite wording is not deployed yet, while the inherited full Flutter suite still reports 22 unrelated UI/theme expectation failures. | Deploy and verify the hosted copy, then separately baseline the inherited Flutter failures before global closure. |
...
| 005-sg-ship | complete | Bounded installer prerequisite copy/test trace passed focused local checks and was pushed to main without staging unrelated paths. | Confirm the matching Vercel deployment, then rerun the live copied-command proof. |
