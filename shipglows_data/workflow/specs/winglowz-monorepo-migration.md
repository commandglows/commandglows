...
Acteurs secondaires: agents ShipGlows, GitHub Actions, Vercel, Firebase CLI, collaborateurs futurs.
...
Resultat observable attendu: `/home/claude/winglowz` contient la racine monorepo, le site vit sous `winglowz_site/`, l'app vit sous `winglowz_app/`, un seul `shipglows_data/` canonique reste a la racine, et les commandes de verification site/app restent executables depuis leurs sous-dossiers.
...
Transformer `winglowz` en racine monorepo, deplacer le site dans `winglowz_site/`, importer l'app dans `winglowz_app/`, puis consolider la gouvernance ShipGlows a la racine. Les workflows, README et chemins de deploiement sont ajustes pour que chaque surface reste deployable et testable depuis son sous-dossier.
...
- Garder un seul `shipglows_data/` a la racine du monorepo.
- Migrer les specs, bugs, reviews, tests logs et docs de gouvernance app depuis `winglowz_app/shipglows_data` vers le corpus racine sans ecraser les documents site existants.
...
- Garder un seul `shipglows_data/` canonique a la racine.
...
- Les specs ShipGlows restent des artefacts internes, pas des contenus Astro.
...
- ShipGlows: `shipglows_data` racine devient le seul corpus de gouvernance.
...
- Les specs et bugs app doivent rester trouvables sous `shipglows_data/workflow/`.
...
- Des artefacts `shipglows_data/business/*.md` existent dans les deux depots; les versions app et site ne doivent pas s'ecraser silencieusement.
...
- [ ] Tache 2 : Creer la structure monorepo site
  ...
  - Action : deplacer les fichiers et dossiers site actuels dans `winglowz_site/` en gardant a la racine seulement les contrats monorepo, `shipglows_data/`, `.github/` si present, et les fichiers de gouvernance racine.
...
- [ ] Tache 4 : Consolider `shipglows_data`
  - Fichiers : `shipglows_data/**`, ancien `winglowz_app/shipglows_data/**`.
  - Action : fusionner les artefacts app dans le corpus racine sans ecraser les artefacts site; conserver les specs app sous `shipglows_data/workflow/specs/`, les bugs sous `shipglows_data/workflow/bugs/`, et les docs techniques/business en les renommant ou scindant si necessaire.
  - Validate with : un seul repertoire `shipglows_data/` dans le monorepo, aucun `winglowz_app/shipglows_data` restant sauf exception documentee.
...
- Il n'existe qu'un `shipglows_data/` canonique a la racine.
...
- Verifier la gouvernance:
  - `find . -path '*/shipglows_data' -type d`
  - `find shipglows_data/workflow/specs -maxdepth 1 -type f | sort`
...
- La fusion `shipglows_data` risque d'ecraser des artefacts app ou site sans convention de renommage claire.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-24 21:02:33 UTC | sf-spec | GPT-5 Codex | Created migration spec from exploration and clean-repo confirmation | draft | /sf-ready shipglows_data/workflow/specs/winglowz-monorepo-migration.md |
| 2026-05-24 21:04:51 UTC | sf-ready | GPT-5 Codex | Validated readiness and added missing behavior/risk sections | ready | /sf-start shipglows_data/workflow/specs/winglowz-monorepo-migration.md |
| 2026-05-24 21:12:12 UTC | sf-start | GPT-5 Codex | Migrated site/app into monorepo, consolidated governance, updated CI/docs, and ran local checks | implemented | /sf-verify shipglows_data/workflow/specs/winglowz-monorepo-migration.md |
| 2026-05-24 21:29:00 UTC | sf-verify | GPT-5 Codex | Verified migrated app history/content, GitHub org transfer, PR checks, and standalone app/snapshot status | completed | /sf-ship shipglows_data/workflow/specs/winglowz-monorepo-migration.md |
| 2026-05-24 21:38:00 UTC | sf-ship | GPT-5 Codex | Merged PR #99 into main and verified post-merge Android CI, Firestore deploy, Firebase Auth check, and APK artifact upload | shipped | /sf-end shipglows_data/workflow/specs/winglowz-monorepo-migration.md |
