...
4. A configured scheduler client with **both** `campaign_dispatch` and `dispatch` permissions pumps one page of one due campaign per tick. Cursor and snapshot→message links commit together; retries cannot create another message for that snapshot. A direct claim reserves one message by default so concurrent workers make quota decisions in separate serializable transactions; the authenticated HTTP worker explicitly requests a bounded batch of ten. Claims retain class priority: operator, service, confirmation, broadcast. A campaign pump failure does not prevent an attempt to dispatch service work.
