...
When Convex delivers webhook alerts to a protected Vercel Preview, configure the transport in the Convex deployment, not in source: `COMMERCE_ALERT_WEBHOOK_URL`, optional `COMMERCE_ALERT_WEBHOOK_BYPASS_TOKEN` for `x-vercel-protection-bypass`, optional `COMMERCE_ALERT_WEBHOOK_TOKEN` for `Authorization: Bearer ...`, and optional comma-separated `COMMERCE_ALERT_WEBHOOK_HEADERS` entries as `Header=Value`. When the email cron calls the protected Astro dispatch worker, configure `EMAIL_WORKER_BYPASS_TOKEN` with the same type of Vercel automation bypass secret. Legacy `VERCEL_AUTOMATION_BYPASS_TOKEN`, `VERCEL_AUTOMATION_BYPASS_SECRET` and `VERCEL_BYPASS_TOKEN` are accepted as fallback bypass names. These headers only prove that Convex can reach the protected route; they do not prove Postmark submission or mailbox receipt.
