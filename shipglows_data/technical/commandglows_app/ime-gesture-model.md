...
depends_on:
  - "shipglows_data/technical/commandglows_app/context.md@1.0.0"
  - "shipglows_data/technical/commandglows_app/context-function-tree.md@1.0.0"
  - "shipglows_data/technical/commandglows_app/code-docs-map.md@0.1.0"
  - "commandglows_app/docs/technical/android-native.md@0.1.0"
supersedes: []
evidence:
  - "CommandGlows IME swipe investigation across CommandGlowsKeyboardView.kt and KeyboardLongPressSwipePolicy.kt."
...
# Technical Behavior Index: CommandGlows IME Gestures

## Purpose

This file owns term-based recovery for CommandGlows IME gesture behavior. It exists because operator terms such as `swipe` are ambiguous in the IME and otherwise force repeated search across native code, docs, tests, and workflow artifacts.

## Canonical Role

- `context.md` stays the system overview for CommandGlows technical surfaces.
...
| --- | --- | --- | --- |
| `swipe` | `long-press-swipe-dispatch` | User holds a key, leaves the origin key, sees the beam, and releases on a target key. | `CommandGlowsKeyboardView.tryActivateLongPressSwipeFromExit` |
...
| `swipe` | `ctrl-hold-swipe-dispatch` | Gesture begins from Ctrl or from the Ctrl launch row. | `CommandGlowsKeyboardView.activateCtrlHoldSwipeMode` |
| `swipe` | `target-corner-selection` | A hovered target key resolves to up/right/down/left gesture meaning. | `CommandGlowsKeyboardView.longPressSwipeSelectionForTarget` |
...
- Summary: after the pointer exits the pressed origin key, CommandGlows can switch into dispatch mode, render the visual beam, track a hovered target key, and dispatch on release instead of forcing the user to toggle a separate mode.
- Entrypoints:
  - `commandglows_app/android/app/src/main/kotlin/com/commandglows_app/commandglows_app/ime/CommandGlowsKeyboardView.kt`
...
- Entrypoints:
  ...
  - `CommandGlowsKeyboardView.canLaunchCtrlSwipeFromFrame`
- Key symbols:
  - `spaceSlideStartPx`
- Tests:
  - `See Android native validation guidance in commandglows_app/docs/technical/android-native.md`
- Specs / bugs:
...
swipe
  ...
  -> CommandGlowsKeyboardView.kt / KeyboardLongPressSwipePolicy.kt
