...
# Platform Behavior — CommandGlows
...
| Platform | Local speech | Advanced recording + Whisper | Secure key storage | Clipboard sync | Overlay / quick actions | CommandGlows keyboard IME |
|---|---|---|---|---|---|---|
...
- CommandGlows keyboard is declared as an Android `InputMethodService` and is configurable from Settings through the `commandglows_app/keyboard` MethodChannel.
...
- The Windows implementation must share Flutter UI, actions, stores, status
  ...
  delivery.
- The first Windows channel is `commandglows_app/windows_overlay`. It exposes
...
- The first Windows implementation should attempt the complete path: global
  hotkey, overlay, selection/clipboard/manual input, shared CommandGlows action,
...
- The channel is `commandglows_app/macos_overlay`. It exposes the shared typed
...
- Accessibility/input-monitoring prompts may affect global hotkey and synthetic
  paste delivery. When macOS blocks either path, CommandGlows must keep the final
...
- Linux overlay is implemented as a first native host version, with runner/manual
  proof still required before any public parity claim.
- The channel is `commandglows_app/linux_overlay`. It exposes the shared typed
